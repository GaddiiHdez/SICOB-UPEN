      ...
      for (const table of TABLES_ORDER) {
        ...
        if (records && records.length > 0) {
          ...
          const formattedRecords = records.map(record => {
            ...
            if (copy.fecha) copy.fecha = new Date(copy.fecha);
            if (copy.fecha_salida) copy.fecha_salida = new Date(copy.fecha_salida);
            if (copy.fecha_estimada) copy.fecha_estimada = new Date(copy.fecha_estimada);
            if (copy.fecha_mantenimiento) copy.fecha_mantenimiento = new Date(copy.fecha_mantenimiento);
            if (copy.proximo_mantenimiento) copy.proximo_mantenimiento = new Date(copy.proximo_mantenimiento);
