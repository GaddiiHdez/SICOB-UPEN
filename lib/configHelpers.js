...
export async function parseNextCorrelativo(plantilla, catAbbr, year, bienModel) {
  ...
  // Obtener solo el registro con el código más alto (desc) para evitar cargar todos en memoria
  ...
  // Extraer el correlativo más alto del único registro obtenido (o 0 si no hay registros)
  let maxCorrelativo = 0;
  if (bienesConPrefijo.length > 0) {
    const match = bienesConPrefijo[0].codigo_inventario.match(regexMatcher);
    if (match) {
      const num = parseInt(match[1], 10);
      if (!isNaN(num)) maxCorrelativo = num;
    }
  }
